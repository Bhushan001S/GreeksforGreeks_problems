...
class Solution {
  public:
    ...
    void findTarget(Node* root,Node* p,int target){
        if(!root) return;
    ...
    int burnTree(Node* root){
        if(!root || root->data ==-1) return 0;
        root->data = -1;
        int lB=burnTree(root->left);
        int rB=burnTree(root->right);
        int pB=burnTree(pair[root]);
        
        return 1+max({lB,rB,pB});
    }
